...
exports.api.post("/uploadTemplate/:categoryId", authentication_1.headerJWTCheck, function (req, res) {
    ...
    if (categoryId) {
        ...
        busboy.on('file', function (fieldname, file, filename, encoding, mimetype) {
            if (mimetype.startsWith('image/')) {
                var imageAttributes = JSON.parse(decodeURI(filename));
                ...
                thumbStream.on('finish', function () {
                    ...
                    var template = new templateModel_2.default({ imageId: tempStream.id, thumbnailId: thumbStream.id, name: filename, categoryId: categoryId, width: imageAttributes.width, height: imageAttributes.height });
...
exports.api.delete('/removeTemplate/:_id', authentication_1.headerJWTCheck, function (req, res) {
    var _id = req.params['_id'];
    templateModel_1.default.remove({ _id: _id }, function (err, count) {
        if (err) {
            res.sendStatus(500);
            console.log(err);
            return;
